        ...
        function get_listado()
	{
        ...
        function get_aulas ($id_sede){
            $sql="SELECT t_a.id_aula, 
                         ...
                         WHERE t_a.id_sede=$id_sede AND (NOT t_a.eliminada)
                         ORDER BY nombre";
            return toba::db('gestion_aulas')->consultar($sql);
        }
        ...
        function get_unidades_academicas (){
            $sql="SELECT t_ua.sigla, t_ua.descripcion
        ...
        function get_sedes ($sigla){
            $sql="SELECT t_s.id_sede, t_s.descripcion
                  FROM sede t_s 
        ...
        function get_aulas_con_capacidad ($id_sede, $capacidad){
            $sql="SELECT nombre as aula, capacidad, id_aula
                  FROM aula 
                  WHERE id_sede=$id_sede AND (capacidad >= $capacidad) AND (NOT eliminada)";
